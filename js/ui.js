...
import { BALANCE, RESOURCE_BASE_YIELD_PER_TILE, RESOURCE_METADATA } from "./balance.js";
import {
  advancedFruitDemand,
...
class GameUI {
  ...
  renderResources() {
    ...
    const advancedRows = isAdvancedMode(this.game) ? `
      ${resourceRow({
        id: "fruit",
        label: "Fruit",
        icon: "🍎",
        value: player.resources.fruit,
        className: flows.fruit < advancedFruitDemand(player) ? "warn" : "good",
        note: `Growth demand ${formatNumber(advancedFruitDemand(player))}/turn`,
        rate: `${signed(flows.fruit)}/turn`,
        tooltip: "Fruit comes from grassland. In Advanced mode it drives population growth and shortages strain stability.",
      })}
      ${resourceRow({
        id: "hardwood",
        label: "Hardwood",
  ...
  projectResourceFlows(player) {
    ...
    for (const tile of this.game.tiles.filter((item) => item.ownerId === player.id)) {
      if (isAdvancedMode(this.game)) {
        const advancedResource = advancedResourceForTile(tile);
        if (advancedResource) flows[advancedResource] += RESOURCE_BASE_YIELD_PER_TILE;
      }
  ...
  renderTechTreeHtml() {
    ...
    const categoryRows = Object.entries(TECH_CATEGORIES).map(([id, config]) => {
      ...
      return `
        <div class="tech-row">
          ...
          <div class="muted">${escapeHtml(description)} Cost: $${formatNumber(cost)}.</div>
          ${transportDetails}
    ...
    const branchRows = this.game.era < 4 ? `<p class="muted">Military branches unlock in Era 4.</p>` : Object.entries(MILITARY_BRANCHES).map(([id, config]) => {
      ...
      const effect = isAdvancedMode(this.game) && id !== "infantry"
        ? `${config.effect} Advanced deployments also consume oil.`
        : config.effect;
      return `
        <div class="tech-row">
          <div class="row-head">
          ...
          <div class="muted">${escapeHtml(effect)} $${formatNumber(cost.money)}, ${cost.materials} materials, ${cost.education} education, ${cost.industry} industry.</div>
  ...
  showActionDeltas(action, tileId, button, result, context = {}) {
    ...
    if (action === "build") {
      ...
      if (result.advancedCost?.oil) deltas.push({ resource: "oil", delta: -result.advancedCost.oil });
...
function projectedFruitGrowth(game, player, flows) {
  const stockAfterFruit = (player.resources.fruit || 0) + flows.fruit;
  const demand = advancedFruitDemand(player);
  if (stockAfterFruit < demand) return 0;
  const headroom = Math.max(0, flows.capacity - player.population.total);
...
function techDescriptionForMode(game, id, description) {
  if (!isAdvancedMode(game)) return description;
  if (id === "farming") return `${description} Fruit from grassland supports population growth.`;
  if (id === "mining") return `${description} Hardwood and iron matter more for construction and factories.`;
  if (id === "education") return `${description} Industry still matters, but advanced factories also need iron.`;
  if (id === "military") return `${description} Advanced branch deployments consume oil.`;
  return description;
}
...
function formatCost(cost, extra = null) {
  ...
  if (extra) {
    for (const resource of ["fruit", "hardwood", "iron", "oil"]) {
      if (extra[resource]) parts.push(`${formatNumber(extra[resource])} ${resource}`);
    }
  }
